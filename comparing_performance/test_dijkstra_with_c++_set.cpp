...
int main(){
     ...
     while(!pq.empty()){
         ...
         for(int i=0;i<adj[nd].size();i++){
             ...
             if(dist[nd]+len < dist[neigh]){
                ...
                pq.insert(make_pair(dist[neigh],neigh));
             }
    ...
    cout << "Time taken by c++ set for dijkstra on this file: "<< duration.count() << " microseconds\n";
     ofstream out;
     out.open("output_dijkstra_with_c++_set.txt");
     for(int i=1;i<n;i++){
         out<<dist[i]<<" ";
     }
     out<<endl;
