...
public:
    long long collision;
    vector<int> size_list={7 ,17, 37, 79, 163, 331, 673, 1361, 2729, 5471, 10949, 21911, 43853, 87719, 175447, 350899, 701819, 1403641, 2807303, 5614657, 11229331};
    int size_list_ptr;
    int sz;
    int occupied;
    ...
    */
    ...
    Set find_intersection(Set<T> other){
        Set<T> intr;
        for(int i=0;i<this->sz;i++){
            if(this->table[i].first == filled){
    ...
    Set find_difference(Set<T> other){
        Set<T> diff;
        for(int i=0;i<this->sz;i++){
            if(this->table[i].first == filled){
                T key=table[i].second;
...
int main(){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int i=0;i<st.table.size();i++){
            cout<<i<<" "<<st.table[i].first<<" "<<st.table[i].second<<"\n";
        }
        cout<<"\n";
    }
    if(st.find(878716) != NULL){
        cout<<"found\n";
    }
    else{
        cout<<"not found\n";
    ...
    if(st.find(878716) != st.end()){
        cout<<"ok found\n";
    }
    else{
        cout<<"not ok not found\n";
    }
*/
    return 0;
